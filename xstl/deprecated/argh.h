...
class ArgumentHandler {
 public:
  ...
  bool ParseArguments(int argc, const char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
      if (argv[i][0] == '-') {
        ...
        if (handler((i + 1 >= argc || argv[++i][0] == '-') ? "" : argv[i]))
          return false;
      }
      else {
  ...
  void AddHandler(const std::string &arg_name, HandFunc handler) {
    handlers_[arg_name] = handler;
  }
  void SetErrorHandler(HandFunc error_handler) {
    error_handler_ = error_handler;
  }
  void AddAlias(const std::string &alias, const std::string &source) {
    if (handlers_[source]) alias_dict_[alias] = handlers_[source];
  }
