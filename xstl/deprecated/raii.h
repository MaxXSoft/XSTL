...
class RAII {
 public:
  ...
  ~RAII() {
    if (destruct_tag_) destruct_();
  }

  RAII &operator=(const RAII &raii) =
...
RAII MakeRAII(Res &res, Construct construct, Destruct destruct) {
  ...
  static_assert(std::is_class<Res>::value,
                "Type Res is not a class or struct type.");
  static_assert(std::is_member_function_pointer<Construct>::value,
                "Type Construct is not a member function.");
  static_assert(std::is_member_function_pointer<Destruct>::value,
                "Type Destruct is not a member function.");

  auto res_addr =
      std::addressof(const_cast<typename RemoveConstant<Res>::type &>(res));
  return RAII(std::bind(construct, res_addr),
              std::bind(destruct, res_addr));
}
